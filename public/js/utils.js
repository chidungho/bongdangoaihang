...
export function parseMatchDateTime(dateValue, timeValue) {
  const baseDate = parseMatchDate(dateValue);
...
export function resolveLiveState(match, now = new Date(), liveWindowMinutes = 120) {
  const kickoff = parseMatchDateTime(match?.date, match?.time);
  const hasScore = String(match?.score || "").trim().length > 0;
  if (!kickoff) return hasScore ? "finished" : "upcoming";
  const liveUntil = new Date(kickoff.getTime() + liveWindowMinutes * 60 * 1000);
  if (hasScore) return "finished";
  if (now >= kickoff && now <= liveUntil) return "live";
